...
export function MainNav({ items, children, logoUrl, siteName, centered }: MainNavProps) {
  ...
  return (
    <div className="flex items-center gap-4 md:gap-6">
      ...
      {pathname === "/" ? (
        <button
          className="absolute left-1/2 -translate-x-1/2 xl:static xl:translate-x-0 flex items-center gap-2.5 shrink-0"
          onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}
        >
          {logoUrl && <img src={logoUrl} alt={siteName ?? "Logo"} className="h-14 w-auto object-contain" />}
          <span className="hidden xl:block font-heading font-bold text-base leading-tight">{siteName ?? "Heo Quay Bình Tân"}</span>
        </button>
      ) : (
        ...
        <nav className={cn(
          ...
          {visibleItems.map((item, index) => {
            ...
            const cls = cn(
              ...
              active
                ? "text-primary font-semibold after:w-full cursor-default"
            ...
            return active || item.disabled ? (
              ...
              <Link key={index} href={item.href} className={cls}>{item.title}</Link>
            )
          })}
                ...
                <div className="absolute left-0 top-full mt-1.5 w-44 border bg-background shadow-lg overflow-hidden z-50 animate-in fade-in-0 zoom-in-95 duration-100">
                  {overflowItems.map((item, index) => {
                    ...
                    const cls = cn(
                      ...
                      active
                        ? "border-primary bg-muted text-foreground font-medium cursor-default"
                    ...
                    return active || item.disabled ? (
                      ...
                      <Link key={index} href={item.href} onClick={() => setShowMore(false)} className={cls}>{item.title}</Link>
                    )
                  })}
                </div>
              )}
      ...
      <button
        type="button"
        className="flex xl:hidden items-center justify-center h-9 w-9 border bg-background hover:bg-muted transition-colors"
